...
#define ANGLE_EPSILON			1e-12 /*1e-6*/
...
/* DJH		 && all_cusps_are_complete(manifold) == TRUE */ )
		{
			/*
	...
	Triangulation	*manifold)
{
	 ...
	 */
/* DJH: it we randomize we may still find the a nice structure.
	if (manifold->solution_type[complete] != geometric_solution
	...
	Triangulation	*manifold)
{
	...
	for (edge = manifold->edge_list_begin.next;
		...
		if (edge->order == 3 && !edge->is_singular )

			if (concave_edge(edge) == TRUE)
			{
				if (three_to_two(edge, &where_to_resume, &manifold->num_tetrahedra, TRUE ) == func_OK)
				{
					my_tilts( manifold );
					return TRUE;
				}
					 ...
					 */
					uFatalError("attempt_three_to_twoA5", "canonize_part_1");
			}
	...
	Triangulation	*manifold)
{
	Tetrahedron	*tet;
	FaceIndex	f,f1;
	EdgeClass	*edge, *where_to_resume;
	...
	for (tet = manifold->tet_list_begin.next;
		...
		{		
			if ( would_create_negatively_oriented_tetrahedra(tet, f) == FALSE && tet != tet->neighbor[f] )
			{
				if (two_to_three(tet, f, &manifold->num_tetrahedra, TRUE ) == func_OK)
				{
					my_tilts(manifold); 
					return TRUE;
				}
					 ...
					 */
					uFatalError("attempt_two_to_threeA3", "canonize_part_1.c");
			}
			else for(f1=0;f1<4;f1++)
			if (f1!=f)
			{
				edge = tet->edge_class[edge_between_faces[f][f1] ];
	                	...
	                	if (edge->order == 3 && !edge->is_singular )
				{
						...
						my_tilts(manifold); 
                	                        return TRUE;
                                       		  ...
                                       		  *      this cannot happen where the hull is concave.
                                     		  */
                                        	uFatalError("attempt_two_to_threeA1", "canonize_part_1");
				}
			}
		}
	...
	FaceIndex	f0)
{
	...
	for (side0 = 0; side0 < 4; side0++)
	{
		...
		if (tet0->dihedral_angle[ultimate][edge_between_faces[f0][side0]]
		  + tet1->dihedral_angle[ultimate][edge_between_faces[f1][side1]]
		  > PI + ANGLE_EPSILON)
