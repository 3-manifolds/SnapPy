...
void remove_bad_vertices(
	Triangulation *manifold )
{
	/* remove edges with one or two cone points (where possible) */

	Cusp		*cusp, *cusp1;
	Boolean		go_again = TRUE;
	int		finite_index, dead_index;
	...
	while (go_again)
	{
		...
		{
			if (cusp->euler_characteristic == 2 && cusp->is_finite == FALSE )
			{
				cusp->is_finite = TRUE;
				dead_index = cusp->index;
				finite_index = 0;
				for(	cusp1 = manifold->cusp_list_begin.next;
					cusp1!=&manifold->cusp_list_end;
					cusp1 = cusp1->next )
					if (	cusp1->index > dead_index )
						cusp1->index--;
					else if (cusp1->is_finite)
						finite_index = MIN( cusp1->index, finite_index );

				cusp->index = finite_index - 1;	
				manifold->num_cusps--;
				manifold->num_or_cusps--;
			}
		}
		else if(	cusp->num_cone_points == 1 && cusp->euler_characteristic == 2 )
		{
			my_drill_tube( manifold, cusp->cone_points[0] );
			go_again = TRUE;
		}
		else	if (	cusp->num_cone_points == 2 && cusp->euler_characteristic == 2 &&
				cusp->cone_points[0] != cusp->cone_points[1] )
		{
			my_drill_tube( manifold, MAX(cusp->cone_points[0],cusp->cone_points[1]) );
			go_again = TRUE;
		}
	}
	...
	int		singular_index )
{
	...
	{
		/* DJH : the cusp may not be unique.  I won't bother changing the name of it! */
		/* DJH : we are not change the number of cusps, just the topology of a $\S^2(n,n)$ orbifold to a torus */
		 ...
		 *	Klein bottle.
		 */
		unique_cusp = tet->cusp[v0];
		unique_cusp->is_complete	= TRUE;	/* to be filled below */
		identify_one_cusp(manifold,unique_cusp);

		if (unique_cusp->topology == Klein_cusp)
		{
			manifold->num_or_cusps--;
			manifold->num_nonor_cusps++;
		}

		if (unique_cusp->num_cone_points!=2)
			uFatalError("my_drill_tube","finite_vertices");

		unique_cusp->euler_characteristic = 0;
		unique_cusp->orbifold_euler_characteristic = 0;
		unique_cusp->num_cone_points = 0;
		my_free(unique_cusp->cone_points);
		unique_cusp->cone_points = NEW_ARRAY( unique_cusp->num_cone_points, int );

		/* DJH : we should set the singular edge to no longer be singular.  We index the singular edges */
		...
		basis_change = NEW_ARRAY( manifold->num_cusps, MatrixInt22 );

		for(i=0;i<manifold->num_cusps;i++)
		{
			basis_change[i][0][0] = 1;
			basis_change[i][1][1] = 1;
			basis_change[i][0][1] = 0;
			basis_change[i][1][0] = 0;
		}


		unique_cusp->cusp_shape[initial] = Zero;	/* force current_curve_basis() to ignore the cusp shape */
		current_curve_basis(manifold, unique_cusp->index, basis_change[unique_cusp->index]);
		if (change_peripheral_curves(manifold, basis_change) != func_OK)
			uFatalError("my_drill_tube", "finite_vertices");

                unique_cusp->is_complete        = TRUE;
                unique_cusp->m                          = 0;
                unique_cusp->l                          = 0;

		my_free( basis_change );
	}
}
