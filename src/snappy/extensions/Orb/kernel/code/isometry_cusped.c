...
static FuncResult	attempt_isometry(Triangulation *manifold0, Tetrahedron *tet0, Tetrahedron *tet1, Permutation map0, int *singular_map );
...
static void			compute_singular_action( Triangulation *manifold0, Isometry *isometry);
	...
	int		*singular_map )
{
	...
	if (	manifold0->num_tetrahedra != manifold1->num_tetrahedra ||
		manifold0->num_singular_arcs != manifold1->num_singular_arcs )
		return FALSE;
	...
	for (tet1 = manifold1->tet_list_begin.next;
		...
		for (i = 0; i < 24; i++)
			 ...
			 *	define an isometry?
			 */
			if (attempt_isometry(manifold0, tet0, tet1, permutation_by_index[i], singular_map ) == func_OK)
				/*
