	...
	int				*num_columns)
{
	if (manifold->orientability == oriented_manifold ||
 		manifold->orientability == oriented_orbifold ) /* DJH */
	{
		real_equations = NULL;
	...
	int				*num_columns)
{
	 ...
	 */									/* DJH */
 	...
 	for ( edge = manifold->edge_list_begin.next;
 		edge != &manifold->edge_list_end; edge = edge->next )
 			edge_eqns++;	


 	*num_rows		=edge_eqns + cusp_eqns ;
	...
	int				*num_columns)
{
	...
	*num_rows		= 2 * (manifold->num_tetrahedra + manifold->num_cusps); /* DJH : this needs changing */
	...
	double			**real_equations)
{
	...
	for (cusp = manifold->cusp_list_begin.next;
		 cusp != &manifold->cusp_list_end;
		 cusp = cusp->next)				/* DJH : should be changed */
	{
	...
	double			*distance_ratio)
{
	...
	*distance_to_solution = ( orientability == oriented_manifold || orientability == oriented_orbifold ) ? /* DJH */
	...
	Complex			*solution)
{
	...
	if (orientability == oriented_manifold || orientability == oriented_orbifold ) /* DJH */
