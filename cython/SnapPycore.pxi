...
# Functions for helping with DirichletDomain to stl construction.

cdef facet_stl(triangle):
...
# This needs to be a def not a cdef because of the yields.
...
cdef midpoint(point1, point2):
    (x1, y1, z1) = point1
    (x2, y2, z2) = point2
    return ((x1+x2) / 2, (y1+y2) / 2, (z1+z2) / 2)

cdef projection(point, cutoff_radius):
    (x, y, z) = point
...
class DirichletDomain(CDirichletDomain):
    ...
    def klein_cutout_stl(self, shrink_factor=0.9):
        for face in self.face_list():
            vertices = face['vertices']
    ...
    def poincare_stl(self, num_subdivisions=5, cutoff_radius=0.9):
        for triangle in subdivide_triangles(self.klein_stl(), num_subdivisions):
    ...
    def poincare_cutout_stl(self, num_subdivisions=3, shrink_factor=0.9, cutoff_radius=0.9):
        for triangle in subdivide_triangles(self.klein_cutout_stl(shrink_factor), num_subdivisions):
            yield (projection(triangle[0], cutoff_radius), projection(triangle[1], cutoff_radius), projection(triangle[2], cutoff_radius))
        return
    
    def stl(self, model='klein', cutout=False, num_subdivisions=3, shrink_factor=0.9, cutoff_radius=0.9):
        """
        Yield the lines of an stl file corresponding to this domain that is suitable for 3d printing.
        ...
        if model == 'klein' and cutout:
            output = self.klein_cutout_stl(shrink_factor=shrink_factor)
        elif model == 'klein' and not cutout:
            output = self.klein_stl()
        elif model == 'poincare' and cutout:
            output = self.poincare_cutout_stl(num_subdivisions=num_subdivisions, shrink_factor=shrink_factor, cutoff_radius=cutoff_radius)
        elif model == 'poincare' and not cutout:
            output = self.poincare_stl(num_subdivisions=num_subdivisions, cutoff_radius=cutoff_radius)
        else:
