...
###############################################################################
# SourceAndObjectFiles for Orb extension

orb_ext_files = SourceAndObjectFiles()

orb_headers = (
    glob(os.path.join('orb', 'kernel', 'snappea', 'headers', '*.h')) +
    glob(os.path.join('orb', 'kernel', 'snappea', 'unix_kit', '*.h')))
orb_ext_files.set_headers(orb_headers)

orb_cython_deps = ['orb/cython/Orb.pxi']
orb_cython_deps += glob(os.path.join('orb', 'cython', '*.pyx'))

orb_ext_files.set_cython_file_language_and_dependencies(
    'orb/cython/Orb.pyx', 'c', orb_cython_deps)

orb_base_code = glob(os.path.join('orb', 'kernel', 'snappea', 'code', '*.c'))
orb_unix_code = glob(os.path.join('orb', 'kernel', 'snappea', 'unix_kit', '*.c'))
