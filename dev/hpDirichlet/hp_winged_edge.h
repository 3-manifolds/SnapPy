...
#ifndef _hp_winged_edge_
#define _hp_winged_edge_
...
/*
 *  The hp_WEEdge structure keeps pointers to Tetrahedra for local use
...
typedef struct hp_WEVertex         hp_WEVertex;
typedef struct hp_WEEdge           hp_WEEdge;
typedef struct hp_WEFace           hp_WEFace;

typedef struct hp_WEVertexClass    hp_WEVertexClass;
typedef struct hp_WEEdgeClass      hp_WEEdgeClass;
typedef struct hp_WEFaceClass      hp_WEFaceClass;

typedef struct hp_WEPolyhedron     hp_WEPolyhedron;


struct hp_WEVertex
{
    /*
     *  The vector x gives the position of the hp_WEVertex in the
     ...
     *  into vertex classes.
     */
    hp_WEVertexClass   *v_class;

    /*
     *  The visible field is used while displaying the hp_WEPolyhedron to
     ...
     *  of the hp_WEVertex's location x[] with an arbitrary but
    ...
    /*
     *  The hp_WEVertices are kept on a doubly-linked list.
     */
    hp_WEVertex        *prev,
                       *next;
};
...
struct hp_WEEdge
{
     ...
     *  tail and tip, respectively, of the directed hp_WEEdge.
     */
    hp_WEVertex     *v[2];

    /*
     *  e[tail][left]  is the hp_WEEdge incident to both v[tail] and f[left].
     *  e[tail][right] is the hp_WEEdge incident to both v[tail] and f[right].
     *  e[tip ][left]  is the hp_WEEdge incident to both v[tip ] and f[left].
     *  e[tip ][right] is the hp_WEEdge incident to both v[tip ] and f[right].
     */
    hp_WEEdge       *e[2][2];

    /*
     *  f[left] and f[right] are the faces incident to the
     *  left and right sides, respectively, of the directed hp_WEEdge.
     */
    hp_WEFace       *f[2];
    ...
    REAL            dist_line_to_origin,
                    dist_edge_to_origin;
    hp_O31Vector    closest_point_on_line,
     ...
     *
     *      edge->neighbor[left] tells the hp_WEEdge to which the given edge
     ...
     *  function Dirichlet_bells_and_whistles() will redirect hp_WEEdges as
     ...
     *  The four Tetrahedra incident to this hp_WEEdge are tet[tail][left],
     *  tet[tail][right], tet[tip][left], and tet[tip][right].
     */
    TetrahedronSneak    *tet[2][2];

    /*
     *  The hp_WEEdges are kept on a doubly-linked list.
     */
    hp_WEEdge       *prev,
                    *next;
...
{
    /*
     *  some_edge is an arbitrary hp_WEEdge incident to the given hp_WEFace.
     */
    hp_WEEdge       *some_edge;

    /*
     *  mate is the hp_WEFace which is identified to this face under
     ...
     */
    REAL            dist;
    hp_O31Vector    closest_point;

    /*
     *  The to_be_removed field is used locally in install_new_face() in
     *  Dirichlet_construction.c to record which hp_WEFaces are to be removed.
     */
    Boolean         to_be_removed;

    /*
     *  The clean field is used locally in check_faces() in
     *  Dirichlet_construction.c to record which hp_WEFaces are known to be
     ...
     *  to record which hp_WEFaces have had their group_elements copied to the
     *  hp_MatrixPairList.
     */
    Boolean         copied;

    /*
     *  The matched field show that the hp_WEEdges incident to this face have
    ...
    /*
     *  The face and its mate are assigned to the same hp_WEFaceClass.
     *  In the case of an orbifold, a face may be its own mate, in which
     *  case the hp_WEFaceClass will have only one element.
     */
    hp_WEFaceClass  *f_class;

    /*
     *  The hp_WEFaces are kept on a doubly-linked list.
     */
    hp_WEFace       *prev,
                    *next;
...
/*
 *  The Dirichlet domain's face pairings identify the hp_WEVertices, hp_WEEdges
 *  and hp_WEFaces into hp_WEVertexClasses, hp_WEEdgeClasses and hp_WEFaceClasses.
 ...
 *  from one another.  This is especially important for the hp_WEFaceClasses.
...
struct hp_WEVertexClass
{
    /*
     *  At present the hp_WEVertexClasses are listed in arbitrary order,
     *  but if necessary they could easily be sorted to provide
     *  some control over their hues, as is done for hp_WEFaceClasses.
     */
    int             index;
    double          hue;
     ...
     */
    hp_WEVertexClass   *belongs_to_region;
    Boolean            is_3_ball;

    /*
     *  The hp_WEVertexClasses are kept on a doubly-linked list.
     */
    hp_WEVertexClass   *prev,
                       *next;
};


struct hp_WEEdgeClass
{
    /*
     *  At present the hp_WEEdgeClasses are listed in arbitrary order,
     *  but if necessary they could easily be sorted to provide
     *  some control over their hues, as is done for hp_WEFaceClasses.
     */
    int             index;
    ...
    /*
     *  The hp_WEEdgeClasses are kept on a doubly-linked list.
     */
    hp_WEEdgeClass  *prev,
                    *next;
};
...
struct hp_WEFaceClass
{
    ...
    /*
     *  Typically a hp_WEFaceClass will have two elements, but if a face is
     *  glued to itself (in an orbifold), then the hp_WEFaceClass will have
     *  only one element.
     */
    int             num_elements;
    ...
    /*
     *  The hp_WEFaceClasses are kept on a doubly-linked list.
     */
    hp_WEFaceClass  *prev,
                    *next;
};
...
struct hp_WEPolyhedron
{
     ...
     */
    hp_WEVertex     vertex_list_begin,
                    vertex_list_end;
    hp_WEEdge       edge_list_begin,
                    edge_list_end;
    hp_WEFace       face_list_begin,
                    face_list_end;
     ...
     */
    hp_WEVertexClass   vertex_class_begin,
                       vertex_class_end;
    hp_WEEdgeClass     edge_class_begin,
                       edge_class_end;
    hp_WEFaceClass     face_class_begin,
                       face_class_end;
