...
static void					compute_one_edge_relation(Triangulation *manifold, EdgeClass *edge, GroupPresentation *group );
	...
	GroupPresentation	*group)
{
	...
	if (get_filled_solution_type(manifold) == nongeometric_solution
	...
	GroupPresentation	*group)
{
	...
	for (edge = manifold->edge_list_begin.next;
		...
		compute_one_edge_relation(manifold,edge, group);
}
	...
	GroupPresentation	*group)
{
	...
	n = (edge->singular_order == 0 ) ? -1 : 0; /* DJH */
	...
	do
	{
		...
		if ( same_positioned_tet(&ptet, &ptet0) == TRUE )
 			n++;

	} while ( n < edge->singular_order ); /* DJH */
	...
	if ( edge->singular_order != 0 &&
		new_word->itsLength != edge->num_incident_generators * edge->singular_order ) /* DJH */
		uFatalError("compute_one_edge_relation", "fundamental_group");
	...
	GroupPresentation	*group)
{
	...
	for ( cusp = manifold->cusp_list_begin.next;
	 	cusp != &manifold->cusp_list_end;	/* DJH */
 		cusp = cusp->next )
	...
	for (cusp->basepoint_tet = manifold->tet_list_begin.next;
			 ...
			 cusp->basepoint_vertex++)
		{
			if (cusp->basepoint_tet->cusp[cusp->basepoint_vertex] != cusp)
				continue;
 			else if ( cusp->topology != torus_cusp && cusp->topology != Klein_cusp ) /* DJH */
 				return;
...
int *fg_get_parabolic_relation(
        ...
        int                                     which_relation)
{
        if (which_relation < 0 || which_relation >= group->itsNumParabolicRelations)
                uFatalError("fg_get_parabolic_relation", "fundamental_group");
...
O31Matrix *get_matrices( GroupPresentation *fg ) /* DJH */
{
 	return fg->itsMatrices;
}

double get_matrix_entry( GroupPresentation *fg, int i, int j, int k ) /* DJH */
{
	return fg->itsMatrices[i][j][k];
}

int get_word_length( GroupPresentation *fg, int word ) /* DJH */
{
 int i;
 CyclicWord *cur;
 ...
 for ( i = 0; i != word; i++, cur = cur->next );

 return cur->itsLength;
}

int get_letter( GroupPresentation *fg, int word, int letter ) /* DJH */
{
 int i;
 CyclicWord *cur_word;
 Letter *cur_letter;

 cur_word = fg->itsRelations;

 for ( i = 0; i != word; i++, cur_word = cur_word->next );

 cur_letter = cur_word->itsLetters;

 for ( i = 0; i != letter; i++ , cur_letter = cur_letter->next);

 return cur_letter->itsValue;
}
/*
extern void compute_reflection_in_face( int index, O31Matrix gen, Tetrahedron *tet )  DJH 
{
  GL4RMatrix     m1,
                 m2,
                 M2;
 ...
 double          length;
        ...
        for(i=0;i<4;i++)
        {
                ...
                if (i==index)
                {
                        length = sqrt( ABS( o31_inner_product(tet->dual_basis[i],tet->dual_basis[i])));
                        if (length < 0.0001) uFatalError("","reflection face...");
                }
                else
                {
                        length = sqrt( ABS( o31_inner_product(tet->basis[i],tet->basis[i])));
                        if (length < 0.0001) length = 1;
                }

                for(j=0;j<4;j++)
                {
                        m1[j][i] = (i==index) ? -tet->dual_basis[i][j] / length :
                                                 tet->basis[i][j] /length;
                        m2[j][i] = (i==index)? m1[j][i] : -m1[j][i]; 
                }
       }
...
extern void compute_reflection_in_truncation_face( int index, O31Matrix gen, Tetrahedron *tet )  DJH 
{
 ...
 double          length=0;
        ...
        for(i=0;i<4;i++)
        {
                ...
                {
                        length = sqrt( ABS( o31_inner_product(tet->basis[i],tet->basis[i])));
                        if (length < 0.0001) uFatalError("","truncation face");
                }
                else
                {
                        length = sqrt( ABS( o31_inner_product(tet->dual_basis[i],tet->dual_basis[i])));
                ...
                for(j=0;j<4;j++)
                {
                        m1[j][i] = (i==index) ? -tet->basis[i][j] / length :
                                                tet->dual_basis[i][j] /length;
                        m2[j][i] = (i==index) ? -m1[j][i] : m1[j][i]; 
                }
       }
