...
def to_matrix(m):
    ...
    Unfortunately, M.fundamental_group().SL2C('a') returns two
    different matrix types (SnapPy.SimpleMatrix and
    SnapPyHP.SimpleMatrix) depending on whether M is a Manifold or
    ManifoldHP. Note that the matrices generated by, e.g.,
    hyperboloid_utilities are always SnapPy.SimpleMatrix. Thus, we can
    get errors when trying to multiply matrices coming from different
    sources.
    """
...
class GeodesicInfo:
    def __init__(self, manifold, word):
        ...
        self.original_matrix = to_matrix(self.group.SL2C(word))
        ...
        # inverses
        generator_matrices = [
            self.normalize_matrix(to_matrix(self.group.SL2C(g)))
            for g in self.group.generators() ]
        generator_matrix_inverses = [
            self.normalize_matrix(to_matrix(self.group.SL2C(g.upper())))
            for g in self.group.generators() ]
    ...
    def find_tet_face_and_matrix_intersecting_geodesic(self):
        ...
        tet = 0
        last_f = -1
        m = to_matrix(self.group.SL2C('')) # Start with identity matrix
