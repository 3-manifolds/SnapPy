	...
	Symmetry	**dest)
{
	...
	*dest					= NEW_STRUCT(Isometry);
	...
	(*dest)->num_singular_arcs	= source->num_singular_arcs;
	...
	(*dest)->singular_image	= NEW_ARRAY(source->num_singular_arcs, int);
	for(i=0;i<source->num_singular_arcs;i++)
		(*dest)->singular_image[i] = source->singular_image[i];
