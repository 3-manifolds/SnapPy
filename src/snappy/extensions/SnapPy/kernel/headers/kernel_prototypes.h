 ...
 *  @brief Declarations for all private kernel functions.
 *
...
/**<
 *  Compute the Chern-Simons value in terms of the fudge factor.
 *  Please see chern_simons.c for details.
 */
extern void compute_CS_fudge_from_value(Triangulation *manifold);
/**<
 *  Compute the Chern-Simons fudge factor.  Please see chern_simons.c
 *  for details.
 */
...
/**<
 *  Uses is_cusp_real to renumber all cusps so that real cusps have consecutive
 *  nonnegative indices beginning at 0 and fake cusps have consecutive negative
 *  indices beginning at -1.
 */

extern void compute_cusp_Euler_characteristics(Triangulation *manifold);
/**<
 *  Computes and stores the Euler characteristic of each cusp.
 */
...
/**<
 *  Returns TRUE if all cusps are complete, FALSE otherwise.
 */

extern Boolean  all_cusps_are_filled(Triangulation *manifold);
/**<
 *  Returns TRUE if all cusps are filled, FALSE otherwise.
 */
...
/**
 *  Low-level routine used mainly within hyperbolic_structure.c, but
 *  also to transfer the Chern-Simons invariant in drilling.c.
 */
/** @{ */
...
/**<
 *  A version of POSIXstrdup using my_malloc.
 */
...
/**<
 *  Complex version of decimal_places_of_accuracy()
 */
...
/**<
 *  One of the low-level functions for
 *
...
/**<
 *  Solve num_rows linear equations in num_columns
 *  variables over the reals.  For more information, see solve_equations.c.
 */

extern FuncResult solve_complex_equations(Complex **complex_equations,
                    int num_rows, int num_columns, Complex *solution);
/**<
 * Complex version of solve_real_equations().
 */
...
/**
 *  Like the usual acos(), asin() or sqrt(), except that almost-legal
 *  values are rounded to legal ones.  E.g. safe_acos(1.00000001) =
 *  acos(1.0) = 0.0, not NaN.
 */
/** @{ */
...
/**<
 *  The inverse hyperbolic sine or cosine, which the standard ANSI
...
/**<
 *  Initialize the fields of a Tetrahedron to the most benign values
 *  possible.
 */
extern void initialize_cusp(Cusp *cusp);
/**<
 *  Initialize the fields of a Cusp to the most benign values
 *  possible.
 */
extern void initialize_edge_class(EdgeClass *edge_class);
/**<
 *  Initialize the fields of an EdgeClass to the most benign values
 *  possible.
 */
