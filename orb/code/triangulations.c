	...
	Triangulation		**manifold_ptr)
{
	...
	*manifold_ptr = NULL;
	...
	manifold->num_singular_arcs			= 0; /* DJH */
	...
	Triangulation	*manifold)
{
	...
	if (manifold != NULL)
	{
		...
		while (manifold->cusp_list_begin.next != &manifold->cusp_list_end)
		{
			...
			if (dead_cusp->cone_points != NULL )
			{
				my_free( dead_cusp->cone_points );
				dead_cusp->cone_points = NULL;
			}

			if (dead_cusp->fundamental_domain != NULL)				/* DJH */
				free_cusp_fundamental_domain( dead_cusp->fundamental_domain );
	...
	*destination = *source;
		 ...
		 tet = tet->next, i++)
	{
		...
		*new_tet[i] = *tet;
		...
		/* DJH: oops. i think this is not needed... */

		for(j=0;j<4;j++)
			for(k=0;k<4;k++)
		{
			new_tet[i]->Gram_matrix[j][k] = tet->Gram_matrix[j][k];
			new_tet[i]->inverse_Gram_matrix[j][k] = tet->inverse_Gram_matrix[j][k];	

			new_tet[i]->basis[j][k] = tet->basis[j][k];
			new_tet[i]->dual_basis[j][k] = tet->dual_basis[j][k];
		}

		for(j=0;j<2;j++)
		{
			new_tet[i]->orientation_parameter[j] = tet->orientation_parameter[j];

			for(k=0;k<6;k++)
			{
				new_tet[i]->use_orientation_parameter[j][k] = tet->use_orientation_parameter[j][k];
				new_tet[i]->dihedral_angle[j][k] = tet->dihedral_angle[j][k];
			}
		}
...
/* DJH don't think this is needed */
		for(j=0;j<2;j++)
			new_edge[i]->inner_product[j] = edge->inner_product[j];
		 ...
		 cusp = cusp->next)
	{
		...
		*new_cusp[cusp->index - min_cusp_index] = *cusp;
		...
		for(j=0;j<2;j++)
			new_cusp[cusp->index - min_cusp_index]->inner_product[j] = cusp->inner_product[j];

		new_cusp[cusp->index - min_cusp_index]->cone_points = NEW_ARRAY(cusp->num_cone_points, int);

		for(j=0;j<cusp->num_cone_points;j++)
			new_cusp[cusp->index - min_cusp_index]->cone_points[j] = cusp->cone_points[j];
	...
	Cusp	*cusp)
{
	...
	cusp->fundamental_domain			= NULL; /* DJH */
...
{
	edge_class->singular_order		= 1; /* DJH */
        edge_class->singular_index              = -1; /* DJH */
        edge_class->is_singular                 = FALSE; /* DJH */
	edge_class->inner_product[ultimate]	= -1;
	edge_class->inner_product[penultimate]	= -1;
	edge_class->order					= 0;
