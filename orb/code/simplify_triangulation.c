...
static void update_tet_angles( Tetrahedron *tet ); /* DJH */
	...
	Triangulation	*manifold)
{
	...
	do
	{
		...
		for (edge = manifold->edge_list_begin.next;
			 edge != &manifold->edge_list_end; /* DJH : if you wish to limit simp do it here */
			 edge = edge->next)
	...
	do
	{
		...
		for (edge = manifold->edge_list_begin.next;
			...
			switch (edge->order)
			{
				...
				case 3:
					if (three_to_two(edge, &where_to_resume, &manifold->num_tetrahedra,FALSE) == func_OK)
					{
						progress = TRUE;
						triangulation_was_simplified = TRUE;
						edge = where_to_resume;
	...
	int			*num_tetrahedra_ptr)
{
	...
	if (tet->shape[complete] != NULL && !edge->is_singular) /* DJH */
	...
	if (
		two_to_three(tet, bottom_face, num_tetrahedra_ptr, FALSE ) == func_failed
	 ||	edge->order != 2
	 ||	cancel_tetrahedra(edge, where_to_resume, num_tetrahedra_ptr) == func_failed
	)
		uFatalError("1remove_edge_of_order_one", "simplify_triangulation");
	...
	FaceIndex	bottom_face)
{
	...
	if (tet->neighbor[bottom_face] == tet)
		return FALSE; /* DJH uFatalError("this_way_works", "simplify_triangulation"); */
	...
	if ( edgeA->is_singular && edgeB->is_singular )
		return FALSE; /* DJH */
	...
	int			*num_tetrahedra_ptr)
{
	...
	if (tet[0]->neighbor[v[0][2]] != tet[0]->neighbor[v[0][3]]
	 || tet[0]->gluing  [v[0][2]] != tet[0]->gluing  [v[0][3]])
		uFatalError("1cancel_tetrahedra", "simplify_triangulation"); 
	...
	if ( outer_edge[0]->is_singular && outer_edge[1]->is_singular) /* DJH */
		return func_failed;
	...
	if (outer_edge[1]->is_singular)
	{
		outer_edge[0]->is_singular = TRUE;
		outer_edge[0]->singular_index = outer_edge[1]->singular_index;
		outer_edge[0]->singular_order = outer_edge[1]->singular_order;
	}
	...
	Boolean keep_shapes )
{
	...
	if (edge->order != 3)
		uFatalError("1three_to_two", "simplify_triangulation");
	...
	if ( edge->is_singular ) return func_failed;/*  DJH */
	...
	Boolean		keep_shapes )
{
	...
	new_class->singular_order				= 1;
	new_class->old_singular_order				= 1;
	new_class->singular_index				= -1;
	new_class->order					= 3;
	...
	if (keep_shapes)
	{
		new_class->inner_product[ultimate] = 
			- tet[0]->Gram_matrix[v[0][0]][v[0][3]]   * tet[1]->inverse_Gram_matrix[v[1][0]][v[1][3]]
			- tet[0]->Gram_matrix[v[0][1]][v[0][3]]   * tet[1]->inverse_Gram_matrix[v[1][1]][v[1][3]]
			- tet[0]->Gram_matrix[v[0][2]][v[0][3]]   * tet[1]->inverse_Gram_matrix[v[1][2]][v[1][3]]
			+ tet[0]->orientation_parameter[ultimate] * tet[1]->orientation_parameter[ultimate];

		new_class->inner_product[ultimate]/= tet[1]->inverse_Gram_matrix[v[1][3]][v[1][3]];
	...
	if (tet[0]->canonize_info != NULL)
	{
		...
		for (i = 0; i < 3; i++)
			for (j = 2; j < 4; j++)
				new_tet[i]->canonize_info->face_status[j] = special_inside_cone_face;
										/* DJH: inside_cone_face */
	}
	...
	int			new_cusp_index)
{
	...
	new_cusp->euler_characteristic = 2;
	new_cusp->orbifold_euler_characteristic = 2;
	new_cusp->num_cone_points = 0;
	new_cusp->cone_points = NULL;
	...
	int			*num_tetrahedra_ptr)
{
	...
	do
	{
		if (ptet.tet->edge_class[edge_between_faces[ptet.near_face][ptet.right_face]]->order == 4 &&
			!ptet.tet->edge_class[edge_between_faces[ptet.near_face][ptet.right_face]]->is_singular ) /* DJH */
		...
		if (ptet.tet->edge_class[edge_between_faces[ptet.near_face][ptet.bottom_face]]->order == 4 &&
			!ptet.tet->edge_class[edge_between_faces[ptet.near_face][ptet.bottom_face]]->is_singular ) /* DJH */
	...
	int			*num_tetrahedra_ptr)
{
	...
	if ( class0->is_singular || class1->is_singular)
		uFatalError( "try_adjacent_fours", "simplify_triangulation"); /* DJH */
	 ...
	 *	order three . . .
	 */
	if (two_to_three(tet0, f0, num_tetrahedra_ptr, FALSE ) == func_failed)
	{
		/*
	 ...
	 *	Tetrahedra by one.
	 */
	if (three_to_two(class0, where_to_resume, num_tetrahedra_ptr, FALSE) == func_failed
	 || three_to_two(class1, where_to_resume, num_tetrahedra_ptr, FALSE) == func_failed)
		uFatalError("try_adjacent_fours", "simplify_triangulation");
	...
	int			*num_tetrahedra_ptr)
{
	...
	if (edge->order != 4 || edge->is_singular)
		return func_failed; /* DJH */
	...
	do
	{
		...
		{
			if (two_to_three(ptet.tet, ptet.near_face, num_tetrahedra_ptr, FALSE ) == func_OK)
			{
				if (three_to_two(edge, where_to_resume, num_tetrahedra_ptr, FALSE) == func_OK)
					return func_OK;
				else
...
static void update_tet_angles( Tetrahedron *tet )
{
	int i,j;
	double w,w1,w2;
	...
	if (gl4R_determinant(tet->Gram_matrix)>0.0001)
		uFatalError("update_tet_angles1","simplify_triangularion");
	...
	if ( tet->edge_class[i]->singular_order != 0 )
		...
		if ( w*w / (w1*w2) > 1.001 || w1*w2 < 0 )
			uFatalError("update_tet_angles2","simpilify_triangulation");

		tet->dihedral_angle[ultimate][i] = safe_acos( w / safe_sqrt( w1*w2 ) );
	}
