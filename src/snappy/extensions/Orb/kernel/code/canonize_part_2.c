...
static void		label_opaque_faces(Triangulation *manifold, Boolean attaching_two_handle );
static void		step_one(Triangulation *manifold, Boolean attaching_two_handle);
static void		initialize_tet_status(Triangulation *manifold);
static Boolean	cone_3_cell(Triangulation *manifold, int *num_finite_vertices, Boolean attaching_two_hanlde);
...
static void		step_two(Triangulation *manifold, Boolean attaching_two_handle);
static Boolean	eliminate_opaque_face(Triangulation *manifold);


void canonical_retriangulation(
	Triangulation	*manifold,
	Boolean		attaching_two_handle )
{
	/*
	...
	if ( !attaching_two_handle && is_canonical_triangulation(manifold) == TRUE)
		return;
	...
	label_opaque_faces(manifold, attaching_two_handle );

	/*
	 *	Carry out the two step retriangulation algorithm described above.
	 */

	step_one(manifold, attaching_two_handle );
	step_two(manifold, attaching_two_handle );

	/*
	 *	Free the CanonizeInfo fields.
	 */

	if (!attaching_two_handle)
...
extern void free_canonize_info(
	Triangulation	*manifold)
{
	Tetrahedron	*tet;
	...
	Boolean		attaching_two_handle )
{
	...
	for (tet = manifold->tet_list_begin.next;
		...
		for (f = 0; f < 4; f++)
		if (attaching_two_handle)
			tet->canonize_info->face_status[f] = opaque_face;
		else
		{
	...
	Boolean		attaching_two_handle)
{
	...
	while (cone_3_cell(manifold, &num_finite_vertices,attaching_two_handle) == TRUE)
		;
}
			...
			if (tet->canonize_info->part_of_coned_cell == TRUE)
				...
				for (f = 0; f < 4; f++)
					...
					if (tet->canonize_info->face_status[f] == transparent_face)
						...
						{
							if (two_to_three(tet, f, &manifold->num_tetrahedra, FALSE ) == func_OK)
